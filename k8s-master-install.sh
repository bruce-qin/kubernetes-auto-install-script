...
read -p "set kubelet cluster ip range(设置kubelet集群api-server服务地址范围，不能与pods地址段有重叠！)[default 172.17.0.0/17]" SERVICE_CLUSTER_IP_RANGE
echo "kubelet cluster ip range:[${SERVICE_CLUSTER_IP_RANGE:=172.17.0.0/17}]"
#安装kube-apiserver
